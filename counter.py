...
class Counter(LaiCiGou):
    def __init__(self, cookie):
        super(Counter, self).__init__(cookie)

        self.types = []

        self.attribute_svg_parser = AttributeSvgParser(cookie, clear=False)
    ...
    # 识别单只狗宝宝的详细属性（有误差）
    def get_baby_attribute_details_api(self, pet_id):
        data = self._query_baby_data(pet_id)
        return self.attribute_svg_parser.predict_one_pet_svg_url_api(data['petUrl'])
